...
%
files={'change4_dec_new.mat','change4_inc_20FOVs.mat','control_lane2_new.mat','control_lane1_new.mat'};
...
for f=1:length(files)
    ...
    head=0;
    for i=1:1
        range=(smedia(i+1)-10):(smedia(i+1)+200+10);
        plot((range)/6,stat(range,1),'linewidth',2);
        % title('Decreasing from 2g/L to 0.5g/L Acetate');
        % title('Decreasing from 2g/L to 0.1g/L NH4Cl');
        % title('Lane 2');
    end
    plot((range)*10/60, 0.059*ones(size(range)),'k--','linewidth',1)
